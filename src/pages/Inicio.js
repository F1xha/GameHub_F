// pages/Inicio.js
// Home con juegos destacados (primeros del dataset)
import React from 'react';
import { Link } from 'react-router-dom';
import Card from '../components/Card';
import juegos from '../data/juegos'; // dataset

export default function Inicio(){
  // Selecciona algunos destacados
          ...
          <Link key={j.id} to={`/juego/${j.id}`} style={{ textDecoration:'none', color:'inherit' }}>
            <Card title={j.title} image={j.images.cover}>
              ...
              <p style={{opacity:.8, fontSize:14}}>
                {j.genre.join(' • ')} | ⭐ {j.rating} | Metascore {j.metascore}
              </p>
            </Card>
          </Link>
        ))}
