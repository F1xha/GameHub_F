...
export default function Admin() {
  ...
  const handleEditar = (juego) => {
    setForm({
        id: juego._id, // 🔥 CORRECCIÓN 1: Usar _id de MongoDB
        title: juego.title,
  ...
  return (
    <div style={{ padding: 20, color: 'white', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 10, background: '#1a1a1a', fontSize: '0.9rem' }}>
        ...
        <tbody>
          {juegos.map(j => (
            // 🔥 CORRECCIÓN 2: Key con _id
            <tr key={j._id} style={{ borderBottom: '1px solid #444' }}>
              ...
              <td style={{padding:10, textAlign:'center'}}>
                ...
                {/* 🔥 CORRECCIÓN 3: Eliminar con _id */}
                <button onClick={() => handleEliminar(j._id)} style={{ cursor:'pointer', border: 'none', background: 'transparent', fontSize: '1.2rem' }} title="Borrar">🗑️</button>
              </td>
            </tr>
          ))}
        </tbody>
